...
**By pattern:** {% for pattern in vignettes | map(attribute='pattern') | unique | sort %}[{{ patterns.get(pattern, pattern) }}](by-pattern/{{ pattern | lower | replace(' ', '-') | replace('/', '-') }}/){.gks-chip} {% endfor %}
